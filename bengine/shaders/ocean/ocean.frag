...
// Water surface shading: Fresnel blend of deep-water colour and Preetham sky
// reflection, sun specular glint, Jacobian foam, and a cheap backlit subsurface
// tint. preethamSky() is duplicated from sky.frag (no shader #include available).
...
uniform sampler2D normalFoam0;
uniform sampler2D normalFoam1;
uniform sampler2D normalFoam2;
...
void main() {
	// combine cascade normal tilts (each n.y ~ 1, n.xz are slopes)
	vec3 n0 = texture(normalFoam0, Uv0).xyz;
	vec3 n1 = texture(normalFoam1, Uv1).xyz;
	vec3 n2 = texture(normalFoam2, Uv2).xyz;
	vec3 N = normalize(vec3(n0.x + n1.x + n2.x, 1.0, n0.z + n1.z + n2.z));

	float foam = clamp(texture(normalFoam0, Uv0).w
	                 + texture(normalFoam1, Uv1).w
	                 + texture(normalFoam2, Uv2).w, 0.0, 1.0);

	vec3 sun = normalize(sunDir);
	vec3 V = normalize(cameraPos - WorldPos);

	vec3 R = reflect(-V, N);
	R.y = abs(R.y);                       // keep reflection above horizon
	vec3 sky = preethamSky(normalize(R), sun, turbidity);
	sky = vec3(1.0) - exp(-exposure * sky);
	...
	vec3 water = mix(deepColor, shallowColor, facing);

	vec3 color = mix(water, sky, fresnel);

	// subsurface tint on backlit crests
	float sss = pow(max(dot(V, -sun), 0.0), 3.0) * smoothstep(-0.2, 0.6, WorldPos.y);
	color += vec3(0.0, 0.32, 0.28) * sss * 0.6;

	// sun specular glint
	vec3 H = normalize(V + sun);
	float spec = pow(max(dot(N, H), 0.0), 220.0);
	color += vec3(1.0, 0.95, 0.8) * spec * 3.0;
